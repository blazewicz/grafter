...
function renderWorktreeDetails(renderDetails: WorktreeDetailsData): string {
  return renderToStaticMarkup(
    createElement(WorktreeDetailsView, {
      homeDirectory: '/repo.worktrees',
      ...displayPreferences,
      details: renderDetails,
      projectWorktrees: [mainWorktree, renderDetails],
      status: 'clean',
...
describe('WorktreeDetails rendering', () => {
  it('renders worktree path card', () => {
    const html = renderWorktreeDetails(details);

    expect(html).toContain('WORKTREE PATH');
  });

  it('renders branch card', () => {
    const html = renderWorktreeDetails(details);

    expect(html).toContain('CHECKED-OUT BRANCH');
  });

  it('renders branch changes card', () => {
    const html = renderWorktreeDetails(details);
  ...
  it('uses a singular file label for a one-file branch comparison', () => {
    const html = renderWorktreeDetails({
      ...details,
